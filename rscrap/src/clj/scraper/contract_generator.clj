...
(defn format-response [response cookie]
  (-> response
      (:body)
      (StringReader.)
      (p/scrap-data)
...
(defn get-request
  ...
  (-> (str (get-base-url) url)
      ...
      (format-response cookie)))



(defn post-request [{:keys [url form-params cookie]}]
  (-> (str (get-base-url) url)
...
(defmulti build-request (fn [request-m _] (get request-m :url)))


(defmethod build-request
  :default
  [request-m params-m]
  (update-in request-m [:form-params] merge (get params-m (:url request-m))))
...
(defmethod build-request
  ...
  [request-m params-m]
  (-> request-m
      (assoc :form-params (get params-m (:url request-m)))
...
(defn send-request
  ...
  ([request-m user-params action-type]
   (let [request (build-request request-m user-params)]
...
(defn batch-request [user-params stop-url]
  (let [v (-> (login-request)
              (send-request user-params)
              (init-flow-request))]
    (loop [request v]
      (cond (or (not-empty (:errormessage request))
                (= stop-url (:url request)))
            request
          ;  (nil? (:url user-params))
           ; request
            :else
            (recur (send-request request user-params))))))
